...
着手前の最終確認で、ログ API の lazy 化方式(案A 全面 lazy 化 / 案B ホットパス限定ガード)を
検討した。「案A は実行が重くなるのでは」という懸念は、ランタイムコストと改修コストを
分けて評価した結果、**ランタイムでは重くならず(重いのは 273 箇所の改修コスト)** と判明。
最終的に **オーバーロードで段階移行する折衷案を採用**した。

検討の全文・比較表・採用コード・受け入れ基準は独立ファイルにまとめた:

→ **`doc/e1_logging_decision.md`**
