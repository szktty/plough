...
void main() {
  ...
  testWidgets('GraphView golden: long labels wrap and render stably', (
    tester,
  ) async {
    final graph = Graph();
    final n1 = GraphNode(
      properties: {
        'label':
            'This is a very very long label for node A that should wrap over multiple lines to test text layout.',
      },
    );
    final n2 = GraphNode(
      properties: {
        'label':
            'Another extremely long label for node B to verify consistent wrapping and ellipsis if any.',
      },
    );
    graph.addNode(n1);
    graph.addNode(n2);
