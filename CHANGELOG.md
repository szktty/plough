...
- [FIX] The force-directed layout no longer overflows the stack when two nodes share a position. Its quadtree subdivided until each body sat alone, but bodies at identical coordinates fall into the same quadrant however small it becomes, so the insert recursed until the stack ran out and took the whole layout down — leaving every node wherever it already was. Nodes are constructed at the origin, so any two that have not been placed yet are enough to trigger it. Subdivision now stops at a depth of 32 and lets bodies share a node; their mass still reaches the centre-of-mass the traversal reads, so forces are unchanged.
- [FIX] `markNeedsLayout` now reaches a view that has finished laying out. `GraphView` only runs a layout while its build state is `performLayout`, and the only route back to that state was a whole new `Graph` instance — so a graph mutated in place, which is what `addNode` on the graph already on screen is for, set `needsLayout` and nothing happened. The added node kept the origin it was constructed at, far outside the viewport: invisible, while links to it were still drawn, since a link only needs its endpoints to exist. A `ready` view that sees `needsLayout` now returns to `performLayout` after the frame.
- [FIX] Pointer handling now ignores non-primary buttons. Selection, dragging and tapping are primary-button gestures, but nothing checked which button was down, so a secondary press ran the whole flow: pressing a node selected it, and pressing empty background armed the deferred deselect that `handlePointerUp` resolves. A host app opening a context menu on right-click therefore lost the selection the menu was about to act on. `PointerUpEvent` reports no buttons, so the up handler remembers whether the press that started the gesture was primary.
- [ADD] `GraphNode.isArranged`: whether a layout pass has placed the node. It already existed on the implementation and is set when a layout finishes; exposing it lets callers tell nodes holding a position worth keeping from ones just added, which is what deciding whom to pin via `nodePositions` requires.
